...
const int NX = 201, NY = 201, NT = 100001;
const double dx = 1, dy = 1; 

// Function to initialize particle density
void initialize_ndis(double *ndis) {
...
void initialize_velocity(double *vf) {
    #pragma omp parallel for
    for (int i = 0; i < NX; i++) {
        for (int j = 0; j < NY; j++) {
            double phase = (i - 100) / 100.0 * M_PI / 2.0; // phase changes from -pi/2 to pi/2 across the grid
            vf[i * NY + j] = sin(phase); // velocity field in x direction, changes from -1 to 1 across the grid
        }
    }
}
...
void solve_equation_TVD(double *ndis, double *temp, double D, double *vf, double dT) {
    #pragma omp parallel 
    {
        for (int t = 0; t < NT; t++) {
            #pragma omp for
            for (int i = 2; i < NX - 2; i++) {
                for (int j = 2; j < NY - 2; j++) {
                    ...
                    double n_ip, flux_ip = 0.0; // i+1/2
                    double n_im, flux_im = 0.0; // i-1/2

                    // ----- i+1/2 -----
                    double v_ip = (vf[i*NY+j] + vf[(i+1)*NY+j]) / 2.0;
                    if (v_ip >= 0.0) {
                        double r_ip = (ndis[i*NY+j] - ndis[(i-1)*NY+j]) / (ndis[(i+1)*NY+j] - ndis[i*NY+j] + 1e-12);
                        double phi_ip = (r_ip + fabs(r_ip)) / (1.0 + fabs(r_ip));
                        n_ip = ndis[i*NY+j] + 0.5 * phi_ip * (ndis[(i+1)*NY+j] - ndis[i*NY+j]);
                    } else {
                        double r_ip = (ndis[(i+1)*NY+j] - ndis[(i+2)*NY+j]) / (ndis[i*NY+j] - ndis[(i+1)*NY+j] + 1e-12);
                        double phi_ip = (r_ip + fabs(r_ip)) / (1.0 + fabs(r_ip));
                        n_ip = ndis[(i+1)*NY+j] + 0.5 * phi_ip * (ndis[i*NY+j] - ndis[(i+1)*NY+j]);
                    }
                    flux_ip = v_ip * n_ip;

                    // ----- i-1/2 -----
                    double v_im = (vf[(i-1)*NY+j] + vf[i*NY+j]) / 2.0;
                    if (v_im >= 0.0) {
                        double r_im = (ndis[(i-1)*NY+j] - ndis[(i-2)*NY+j]) / (ndis[i*NY+j] - ndis[(i-1)*NY+j] + 1e-12);
                        double phi_im = (r_im + fabs(r_im)) / (1.0 + fabs(r_im));
                        n_im = ndis[(i-1)*NY+j] + 0.5 * phi_im * (ndis[i*NY+j] - ndis[(i-1)*NY+j]);
                    } else {
                        double r_im = (ndis[i*NY+j] - ndis[(i+1)*NY+j]) / (ndis[(i-1)*NY+j] - ndis[i*NY+j] + 1e-12);
                        double phi_im = (r_im + fabs(r_im)) / (1.0 + fabs(r_im));
                        n_im = ndis[i*NY+j] + 0.5 * phi_im * (ndis[(i-1)*NY+j] - ndis[i*NY+j]);
                    }
                    flux_im = v_im * n_im;
            ...
            #pragma omp single
            {
                ...
                if (!(t % 100)){
                    char filename1[50];
                    snprintf(filename1, sizeof(filename1), "ndis_vf_ci_TVD_2D_anime.bin");
                    write_array_to_bin(filename1, ndis, NX * NY);
                }
                ndis[NX / 2 * NY + NY / 2] += 1.0; // source term at the center, note that this source term is added after the boundary condition, so it will not be affected by the boundary condition.
            }
        }
    }
}
...
int main(void){
    ...
    double *vf = (double *)malloc(NX * NY * sizeof(double));
    ...
    for (int e = 0; e < 1; e++) {
        ...
        initialize_ndis(ndis);
        initialize_to_0(temp);
        initialize_velocity(vf);
        // Solve the diffusion equation
        solve_equation_TVD(ndis, temp, D, vf, dT);
