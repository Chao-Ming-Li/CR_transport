...
int main() {
    ...
    solve_diffusion_equation(ndis_C, ndis_B, ndis_H, D, Rg, DT); // Rg = 1.0 GV
    write_array_to_bin("ndis_C_1GeV_2nd_order_gc1.bin", ndis_C, (NR + 4) * (NZ + 4));
    write_array_to_bin("ndis_B_1GeV_2nd_order_gc1.bin", ndis_B, (NR + 4) * (NZ + 4));

    // solve_advection_equation(ndis_C, ndis_B, ndis_H, vR, vZ);
    // write_array_to_bin("ndis_C_1GeV_vbreeze1000.bin", ndis_C, NR * NZ);
    // write_array_to_bin("ndis_B_1GeV_vbreeze1000.bin", ndis_B, NR * NZ);    
