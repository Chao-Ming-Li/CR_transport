...
const double dR1 = 0.1, dZ1 = 0.02, dR2 = 0.5, dZ2 = 0.1, R_disk = 10.0, Z_disk = 0.1;
...
void initialize_velocity(double *vR, double *vZ) {
    ...
    for (int i = 0; i < NR; i++) {
        for (int j = 0; j < NZ; j++) {
            ...
            double temp = exp(- 1. / r) / r / r; // velocity field in x direction, changes from -1 to 1 across the grid
            vR[i * NZ + j] = temp * R[i] / r;
            vZ[i * NZ + j] = temp * Z[j] / r;
        }
    }
}
...
void initialize_H(double *ndis_H, const double nH) {  
// nH = 0.0: no spallation; nH > 0: flat hydrogen distribution; nH = -1.0:we take the distribution of H_2 and HI from doi:10.1093/mnras/staa1017, and HII from doi: 10.1111/j.1365-2966.2004.08349.x; nH = -2.0: we use the BronfFerr gas model in dragon2 code
    if (nH >= 0.0){
        for (int i = 0; i < NR; i++) {
            for (int j = 0; j < NZ; j++) {
                ndis_H[i*NZ+j] = nH;
        ...
        for (int i = 0; i < NR; i++) {
            for (int j = 0; j < NZ; j++) {
                ...
                ndis_HI[i*NZ+j] = ratio * 53. / 4. / 85. * exp(-4./R[i] - R[i] / 7) / pow(cosh(Z[j] / 2. / 0.085), 2);
                ndis_HII[i*NZ+j] = 0.00015 * (1. + 3.7*log(1 + r0 / 20.) / (r0/20.) - 1.0277);
                if (R[i] >= 7.8 && R[i] <= 8.2 && Z[j] < 0.2){ // local bubble with radius and 200 pc height, nH = 0.05 cm^-3
                    ndis_HII[i*NZ+j] = 0.05;
                }
                else{
                    ndis_H[i*NZ+j] =  2.0 * ndis_H2[i*NZ+j] + ndis_HI[i*NZ+j] + ndis_HII[i*NZ+j];
                }
        ...
        for (int i = 0; i < NR; i++) {
            for (int j = 0; j < NZ; j++) {
                double z_H2 = (R[i] > 2.5) ? 0.059 : 0.015;
                double X_CO = 0.9 * exp(R[i] / 7.1) + 0.5 * exp(- R[i] / 3.0);
                ndis_H2[i*NZ+j] = (150. * exp(-pow((R[i] - 0.05) / 0.2, 2)) + 0.49 * exp(-pow((R[i] - 5.3) / 2.2, 2)) + 0.24 * exp(-pow((R[i] - 8.1) / 1.5, 2))) * exp(-log(2) * pow(Z[j] / z_H2, 2)) / X_CO;
                double z_HI;
                if (R[i] < 2.5) z_HI = 0.045;
                else if (R[i] < 8.5) z_HI = 0.115;
                else z_HI = 0.115 * exp((R[i] - 8.5) / 6.7);
                double n0;
                if (R[i] < 3.0) n0 = 0.57 * exp((R[i] - 3.0) / 0.4) + 8.0 * exp(-pow(R[i] / 0.2, 2.0));
                else if (R[i] < 13.0) n0 = 0.57;
                else n0 = 0.57 * exp(-(R[i] - 13.0) / 4.0);
                ndis_HI[i*NZ+j] = n0 * exp(- log(2) * pow(Z[j] / z_HI, 2));

                // ndis_HII[i*NZ+j] = 0.025 * exp(- pow(R[i] / 20.0, 2) - Z[j] / 1.0) + 0.2 * exp(- pow((R[i] - 4.0) / 2.0, 2) - Z[j] / 0.15);

                if (R[i] >= 7.8 && R[i] <= 8.2 && Z[j] < 0.2){ // local bubble with radius and 200 pc height, nH = 0.05 cm^-3
                    ndis_H[i*NZ+j] = 0.05;
                }
                else{
                    ndis_H[i*NZ+j] =  2.0 * ndis_H2[i*NZ+j] + ndis_HI[i*NZ+j];
                }
            }
        }
        free(ndis_H2); free(ndis_HI); // free(ndis_HII);                
        ...
        for (int i = 0; i < NR; i++) {
            for (int j = 0; j < NZ; j++) {
                double X_CO = 0.9 * exp(R[i] / 7.1) + 0.5 * exp(- R[i] / 3.0);
                ndis_H2[i*NZ+j] = .94*(11.2*exp(-(R[i])*(R[i])/.874)+.83*exp(-pow((R[i]-4)/3.2, 2.)))*exp(-log(2)*pow(Z[j]/(1.06/1000.*(10.8*exp(0.28*(R[i]))+42.78)), 2.)) / X_CO;
                double height_H1 = 1.06*(116.3 + 19.3*(R[i]) +  4.1*(R[i])*(R[i]) -0.05*(R[i])*(R[i])*(R[i]))/1000.;
                ndis_HI[i*NZ+j] =  .94*(.6*exp(-R[i]/2.4) + .24*exp(-pow((R[i]-9.5)/4.8, 2.)))*exp(-log(2)*pow(Z[j]/height_H1, 2.));
...
void solve_diff_adv_equation_CN(double *ndis_C12, double *ndis_B11, double *ndis_H, double D, double Rg) {
    ...
    initialize_to_disk(src_C12, 1.0, R_disk); 
    ...
    char fn0[100], fn1[100], fn2[100], namebody[80];
    snprintf(namebody, sizeof(namebody), "%dGV_R%d_Z%d_H001_t1e10.bin",(int)Rg, (int)round(R[NR-1]), (int)round(Z[NZ-1]));

    for (int t = 0; t < NT; t++){
        // sigma is cross-section matrix defined above, sigma[a][a] is the total inelastic cross-section of a
        CN_scheme(ndis_C12, src_C12, ndis_H, dT_tau, temp, beta_C12, sigma[C12][C12]* enhancement_factor, tau_stable, beta_C12 * D);

        for (int idx = 0; idx < NR * NZ; idx++){
            src_B11[idx] = ndis_C12[idx] * sigma[C12][B11]* beta_C12 * ndis_H[idx] * l_year* enhancement_factor;  
        }

        CN_scheme(ndis_B11, src_B11, ndis_H, dT_tau, temp, beta_B11, sigma[B11][B11]* enhancement_factor, tau_stable, beta_B11 * D);

        if (t % 1000 == 0 && t != 0){ 
            snprintf(fn0, sizeof(fn0), "ndis_C12_%s", namebody);
            snprintf(fn1, sizeof(fn1), "ndis_B11_%s", namebody);
            write_array_to_bin(fn0, ndis_C12, NR*NZ);           
            write_array_to_bin(fn1, ndis_B11, NR*NZ);  
            printf("Time step is %d,", t);
        }
    }
...
void CN_scheme_opt(double *ndis, double *src, double *dT_tau, CNMatrix *matrix, CNWorkspace *workspace) {
    ...
    double CN_Rb_iso[NR], CN_Rd[NR], CN_Rx[NR];
    // Step 1: Implicit R, Explicit Z
    for (int k = 1; k < NZ - 1; k++) {     
        for (int j = 1; j < NR - 1; j++) {
            CN_Rd[j] = matrix->alpha_Z[k] / dZ[k] * ndis[j * NZ + k - 1] + (1.0 - matrix->alpha_Z[k] * (dZ[k + 1] + dZ[k]) / dZ[k + 1] / dZ[k]) * ndis[j * NZ + k] + matrix->alpha_Z[k] / dZ[k + 1] * ndis[j * NZ + k + 1] - ndis[j * NZ + k] * dT_tau[j * NZ + k] / 4.0;
            CN_Rb_iso[j] = matrix->CN_Rb[j] + dT_tau[j * NZ + k] / 4.0;
        }
        CN_Rb_iso[0] = 1.0;
        CN_Rd[0] = 0.0;
        thomas_solve(matrix->CN_Ra, CN_Rb_iso, matrix->CN_Rc, CN_Rd, CN_Rx, workspace->CN_Rcp, workspace->CN_Rdp, NR - 1);
        CN_Rx[NR - 1] = 0.0; // enforce absorbing boundary at Rmax
        for (int j = 0; j < NR; j++) workspace->ndis_temp[j * NZ + k] = CN_Rx[j];
    }
    apply_boundary_conditions(workspace->ndis_temp);

    // Step 2: Implicit Z, Explicit R
    double CN_Zb_iso[NZ],CN_Zd[NZ], CN_Zx[NZ];
    for (int j = 1; j < NR - 1; j++){
        for (int k = 1; k < NZ - 1; k++){
            CN_Zd[k] = matrix->alpha_R[j] * (1.0 / dR[j] - 0.5 / R[j]) * workspace->ndis_temp[(j-1) * NZ + k] + (1.0 - matrix->alpha_R[j] * (dR[j + 1] + dR[j]) / dR[j + 1] / dR[j]) * workspace->ndis_temp[j * NZ + k] + matrix->alpha_R[j] * (1.0 / dR[j + 1] + 0.5 / R[j]) * workspace->ndis_temp[(j+1) * NZ + k] - workspace->ndis_temp[j * NZ + k] * dT_tau[j * NZ + k] / 4.0;
            CN_Zb_iso[k] = matrix->CN_Zb[k] + dT_tau[j * NZ + k] / 4.0;

        }
        CN_Zb_iso[0] = 1.0;
        CN_Zd[0] = 0.0;
        thomas_solve(matrix->CN_Za, CN_Zb_iso, matrix->CN_Zc, CN_Zd, CN_Zx, workspace->CN_Zcp, workspace->CN_Zdp, NZ - 1);
        CN_Zx[NZ - 1] = 0.0; // enforce absorbing boundary at ZH
        for (int k = 0; k < NZ; k++) ndis[j * NZ + k] = CN_Zx[k] + src[j * NZ + k];
    }
    apply_boundary_conditions(ndis);
...
void solve_diff_adv_equation_CN_opt(double *ndis_C12, double *ndis_B11, double *ndis_H, double D, double Rg) {
    ...
    CNMatrix matrix;
    CNWorkspace workspace;
    double *src_C12 = malloc(NR * NZ * sizeof(double));
    double *src_B11 = malloc(NR * NZ * sizeof(double));
    double *dT_tau_C = malloc(NR * NZ * sizeof(double));
    double *dT_tau_B = malloc(NR * NZ * sizeof(double));
    workspace.ndis_temp = malloc(NR * NZ * sizeof(double));


    initialize_to_disk(src_C12, 1.0, R_disk); 
    initialize_to_0(src_B11);
    initialize_to_0(workspace.ndis_temp);
    ...
    char fn0[100], fn1[100], fn2[100], namebody[80];
    snprintf(namebody, sizeof(namebody), "%dGV_R%d_Z%d_H001_t1e10_opt1.bin",(int)Rg, (int)round(R[NR-1]), (int)round(Z[NZ-1]));

    // define ADI coefficients, these coefficients are constant for a given D and time step dT, so we can compute them once before the time loop 

    for (int i = 0; i < NR - 1; i++) matrix.alpha_R[i] = D * dT / (dR[i] + dR[i + 1]) ;
    for (int i = 0; i < NZ - 1; i++) matrix.alpha_Z[i] = D * dT / (dZ[i] + dZ[i + 1]) ;

    // in R direction
    for (int j = 1; j < NR - 1; j++){     // cylinderical coordinate system  
        matrix.CN_Ra[j] = - matrix.alpha_R[j] * (1.0 / dR[j] - 0.5 / R[j]);
        matrix.CN_Rb[j] = 1.0 + matrix.alpha_R[j] * (dR[j + 1] + dR[j]) / dR[j + 1] / dR[j];
        matrix.CN_Rc[j] = - matrix.alpha_R[j] * (1.0 / dR[j + 1] + 0.5 / R[j]);
    }
    matrix.CN_Ra[0] = 0.0;
    matrix.CN_Rc[0] = -1.0;
    ...
    // in Z direction
    for (int k = 1; k < NZ - 1; k++){
        matrix.CN_Za[k] = - matrix.alpha_Z[k] / dZ[k];
        matrix.CN_Zb[k] = 1.0 + matrix.alpha_Z[k] * (dZ[k + 1] + dZ[k]) / dZ[k + 1] / dZ[k];
        matrix.CN_Zc[k] = - matrix.alpha_Z[k] / dZ[k + 1];
    }
    matrix.CN_Za[0] = 0.0;
    matrix.CN_Zc[0] = -1.0;
    
    // fragmentation and decay term, these terms are constant for a given D, Rg, and time step dT, so we can compute them once before the time loop
    for (int idx = 0; idx < NR * NZ; idx++) {            
        dT_tau_C[idx] = dT * yr2s * (ndis_H[idx] * sigma[C12][C12] * enhancement_factor * beta_C12 * c); 
        dT_tau_B[idx] = dT * yr2s * (ndis_H[idx] * sigma[B11][B11] * enhancement_factor * beta_B11 * c); 
    } 

    for (int t = 0; t < NT; t++){
        CN_scheme_opt(ndis_C12, src_C12, dT_tau_C, &matrix, &workspace);

        for (int idx = 0; idx < NR * NZ; idx++){
            src_B11[idx] = ndis_C12[idx] * sigma[C12][B11]* beta_C12 * ndis_H[idx] * l_year* enhancement_factor;  
        }

        CN_scheme_opt(ndis_B11, src_B11, dT_tau_B, &matrix, &workspace);
